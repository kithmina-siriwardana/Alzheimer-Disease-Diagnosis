...
export default function EmotionDetectionPanel() {
  ...
  // Handle analysis
  const handleAnalyze = () => {
    if ((detectionMode === 'face' && !faceFile) || (detectionMode === 'speech' && !audioFile)) return;
    ...
    setTimeout(() => {
      ...
      const mockResult = {
        ...
        details: {
          secondaryEmotions: [
            { name: 'Confidence', value: (0.6 + Math.random() * 0.3).toFixed(2) },
            { name: 'Intensity', value: (0.5 + Math.random() * 0.4).toFixed(2) },
  ...
  const getResponseForEmotion = (emotion: string): string => {
    const responses: Record<string, string> = {
      'Happy': "That's wonderful! Your happiness is clearly visible. What positive event has contributed to this joyful state?",
      'Sad': "I notice you seem sad. It's okay to feel this way. Would you like to talk about what's bothering you?",
      'Angry': "I can see you're feeling frustrated or angry. Taking deep breaths might help. Would you like to discuss what's causing these feelings?",
      'Neutral': "You appear to be in a calm, neutral state. How are you feeling inside?",
    ...
    return responses[emotion] || "I'd like to understand how you're feeling better. Could you tell me more?";
  };
  
  // Helper function to get recommendations based on emotions
  const getRecommendationForEmotion = (emotion: string): string => {
    const recommendations: Record<string, string> = {
      'Happy': "Continue with activities that bring joy. Consider journaling about these positive moments to revisit when feeling low.",
      ...
      'Angry': "Try relaxation techniques such as deep breathing or counting to ten. Consider stepping away from triggering situations when possible.",
      'Neutral': "This is a good time for mindfulness practice or reflection. Consider setting intentions for the day or week ahead.",
      ...
      'Surprise': "Take a moment to process the unexpected event. Consider how this might lead to new opportunities or perspectives."
    };
    
    return recommendations[emotion] || "Regular check-ins with your emotions can help build emotional awareness and resilience.";
  };
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div className="md:col-span-2 bg-white rounded-lg shadow-md p-6">
        <h2 className="text-xl font-semibold mb-4">
          {detectionMode === 'face' ? 'Upload Facial Image' : 'Record Speech Sample'}
        </h2>
              ...
              <div className="flex flex-col items-center space-y-4">
                <div className="w-full p-6 bg-gray-50 rounded-lg flex flex-col items-center justify-center">
                  <div className={`w-24 h-24 rounded-full flex items-center justify-center ${isRecording ? 'bg-red-100 animate-pulse' : 'bg-gray-100'}`}>
                    <svg xmlns="http://www.w3.org/2000/svg" className={`h-12 w-12 ${isRecording ? 'text-red-600' : 'text-gray-400'}`} viewBox="0 0 20 20" fill="currentColor">
                      <path fillRule="evenodd" d="M7 4a3 3 0 016 0v4a3 3 0 11-6 0V4zm4 10.93A7.001 7.001 0 0017 8a1 1 0 10-2 0A5 5 0 015 8a1 1 0 00-2 0 7.001 7.001 0 006 6.93V17H6a1 1 0 100 2h8a1 1 0 100-2h-3v-2.07z" clipRule="evenodd" />
                    </svg>
                  </div>
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <div className="space-y-4">
          <div className="flex">
            ...
            <div className="ml-4">
              <h3 className="text-lg font-medium text-gray-900">Upload or Record</h3>
          ...
          <div className="flex">
            ...
            <div className="ml-4">
              ...
              <p className="mt-1 text-sm text-gray-500">
                Our AI model will analyze the {detectionMode === 'face' ? 'facial expression' : 'voice pattern'} to detect emotional states.
              </p>
            </div>
          </div>
          <div className="flex">
            ...
            <div className="ml-4">
              ...
              <p className="mt-1 text-sm text-gray-500">
                View the detected emotion and recommended responses for caregivers.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 p-4 bg-yellow-50 rounded-md">
          <div className="flex">
            ...
            <div className="ml-3">
              <h3 className="text-sm font-medium text-yellow-800">Note for Caregivers</h3>
              <div className="mt-2 text-sm text-yellow-700">
                <p>
                  This tool is designed to assist in patient care by helping identify emotional states that may be difficult to express verbally. Always combine these insights with your professional judgment.
                </p>
              </div>
            </div>
          </div>
